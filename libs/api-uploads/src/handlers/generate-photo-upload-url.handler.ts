...
import { assertOwnership, getOwnerId, parseJson, guessExtension, nowPlusSeconds, PETTZI_DOCS_BUCKET_NAME } from './common';
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    const key = `pets/${petId}/photos/${fileId}${guessExtension(contentType)}`;
    ...
    console.error('Generate photo upload URL error', error);
